...
export default function SocialIcons({ socialMedia, className = '' }: SocialIconsProps) {
  ...
  const getTooltipText = (name: string): string => {
    const tooltips: Record<string, string> = {
      Instagram: '@nagira_farm',
      Facebook: '@nagira.farm',
      YouTube: '@nagirafarm',
      Tiktok: '@nagirafarm'
    };
    return tooltips[name] || '@nagira_farm';
  };
    ...
    <div className={`flex space-x-4 ${className}`}>
      {socialMedia.map((social, idx) => (
        <div key={idx} className="relative group">
          ...
          {/* Tooltip */}
          <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs text-white bg-green-800 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap pointer-events-none z-10 shadow-lg">
            {getTooltipText(social.name)}
          </span>
        </div>
      ))}
    </div>
